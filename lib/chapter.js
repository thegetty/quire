...
class Chapter {
  ...
  epubLink(href) {
    ...
    let chapterName = upath
      ...
      .replace(/\/$/, '')
      .replace('/', '__')
    chapterName = chapterName !== '' ? `${chapterName}` : 'cover'
    return chapterName
  }
  ...
  reformatPage() {
    ...
    function reformatIDs() {
      //if (chapter.title.trim() === 'Contents') {
      chapter.content
        .find('[id]')
    ...
    function reformatLinks() {
      ...
      // Decode any urls and Convert any " " characters in anchor links to "-"
      // if (chapter.title.trim() === 'Contents') {
      chapter.links
        .filter((index, el) => {
