...
class Chapter {
  ...
  pandocTitle(title) {
    ...
    title = title !== this.epubTitle ? title.replace(/[^a-zA-Z0-9 ]/g, ' ') : title.replace(/\’/g, '').replace(/[^a-zA-Z0-9.]/g, ' ')
    let t = title
      // lowercase
      .toLowerCase()
  ...
  epubLink(href) {
    ...
    chapterName = chapterName !== '' ? `${chapterName}` : `#${this.pandocTitle(this.epubTitle)}`
  ...
  determineChapterId() {
    ...
    if (this.content.find('.quire-contents-header > h1')) {
      if (this.content.find('.quire-contents-header > h1').attr('id') !== undefined) {
        id = this.content.find('.quire-contents-header > h1').attr('id')
      }
    }

    if (this.content.find('h1.quire-page__header__title')) {
      if (this.content.find('h1.quire-page__header__title').attr('id') !== undefined) {
  ...
  reformatPage() {
    ...
    function reformatH1IDsForPandoc() {
      chapter.content.find('h1')
        .each((index, el) => {
          let id = chapter.$(el).attr('id')
          if (id !== undefined) {
            id = id.toLowerCase().replace(/[^a-zA-Z0-9 ]/g, " ").replace(/\s\s+/g, ' ').replace(/\s/g, "-")
          } else {
            id = 'cover'
          }
    ...
    function reformatLinks() {
      ...
      chapter.links
        ...
        .attr('href', (index, href) => {
          return href.replace('#cover', `#${chapter.pandocTitle(chapter.epubTitle)}`)
        })
        ...
        .attr('href', (index, href) => {
          ...
          if (href !== `${chapter.pandocTitle(chapter.epubTitle)}`) {
            hrefArr.push({ 'internalLinks': `#${href}-${chapter.pandocTitle(chapter.chapterId)}`, 'mdPage': `${chapter.pandocTitle(chapter.chapterId)}` })
