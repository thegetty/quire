...
const CONFIG = {
  ...
  CURRENT_VERSION: 'v0.1.0.alpha.13',
...
class CLI extends EventEmitter {
  ...
  */
  ...
  epub(filename, filePath, env, download) {
    let project = new Project(this.verbose)
    project.on('info', this.notice)
    project.on('error', this.error)
    project.buildEpub(filename, filePath, env)
    if (download) {
  ...
  mobi(filename, filePath, env) {
    let project = new Project(this.verbose)
    project.on('info', this.notice)
    project.on('error', this.error)
    project.buildMobi(filename, filePath, env)
  }
  ...
  pdf(filename, filePath, env) {
    let project = new Project(this.verbose)
    project.on('info', this.notice)
    project.on('error', this.error)
    project.buildPDF(filename, filePath, env)
  }
