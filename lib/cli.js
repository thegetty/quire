  ...
  create(projectName) {
    ...
    async function runGit(themeDir, projectDir, self, reject, resolve) {
      try {
        ...
        if (process.env.THEME_SSH_KEY_FILE_NAME) {
          console.log(`top ${process.env.THEME_SSH_KEY_FILE_NAME}`);
          // execSync(`ssh-agent -D`);
          execSync(`ssh-add ~/.ssh/${process.env.THEME_SSH_KEY_FILE_NAME}`);
          execSync(`cat ~/.ssh/config`);
          await execa("git", ["submodule", "sync"]);
          execSync(
            `"ssh -i ~/.ssh/${process.env.THEME_SSH_KEY_FILE_NAME}" git submodule update --init --recursive --remote`
          );
          console.log(`bottom ${process.env.THEME_SSH_KEY_FILE_NAME}`);
        } else {
          await execa("git", ["submodule", "sync"]);
          await execa("git", [
            "submodule",
