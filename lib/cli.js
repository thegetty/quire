    ...
    async function runGit(themeDir, projectDir, self, reject, resolve) {
      try {
        ...
        if (process.env.THEME_SSH_KEY_FILE_NAME) {
          /**
           * Circleci Note
           * Set the host gettygithub.com to pick the
           * pickup the private key created for circleci.
           * This is only used if the enviroment variable
           * is set by circleci. Do not set THEME_SSH_KEY_FILE_NAME
           * enviroment variable in a local or other remote enviroment
          ...
          await execa("git", [
            ...
            "git@gettygithub.com:gettypubs/quire-starter-theme.git"
          ]);
        } else {
          await execa("git", [
            "config",
