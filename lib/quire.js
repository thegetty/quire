...
class QuireCLI extends EventEmitter {
  ...
  pdf() {
    this.notice('The `pdf` command still needs to be implemented.')
  }
  ...
  _preflight() {
    ...
    let config = this._readYAML(path.join(process.cwd(), 'config.yml'))

    return {
      name: config.theme,
      path: path.join(process.cwd(), 'themes', config.theme)
    }
  }
