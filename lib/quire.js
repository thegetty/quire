...
const CONFIG = {
  ...
  THEME_REPO_BASEURL: 'https://github.com/gettypubs/quire-starter-theme',
...
class QuireCLI extends EventEmitter {
  ...
  create(projectName) {
    ...
    // Clone kit and theme submodule
    this.notice('Installing starter kit...')
    this._gitCloneRecursive(CONFIG.STARTER_REPO, projectName)
    this.confirm('Starter kit successfully installed.')

    // Flatten submodule into main project, install theme deps via npm
    this.notice('Setting up theme...')
    process.chdir(projectName)
    let theme = this._readYAML(path.join(process.cwd(), 'config.yml')).theme
    let themePath = path.join('themes', theme)
    spawnSync('git', ['remote', 'rm', 'origin'])
    this._flattenSubmodule(themePath)
  ...
  _flattenSubmodule(submodulePath) {
    spawnSync('git', ['rm', '.gitmodules'])
    spawnSync('git', ['rm', '--cached', submodulePath])
    spawnSync('rm', ['-rf', `${submodulePath}/.git`])
    spawnSync('git', ['add', submodulePath])
    spawnSync('git', ['add', '-A'])
  ...
  _gitCloneRecursive(repo, destination) {
    spawnSync('git', ['clone', '--recursive', repo, destination], {
      // stdio: 'inherit'
    }, function(err) { if (err) this.emit('error', err) })
  }
