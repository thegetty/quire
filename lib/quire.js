...
class QuireCLI extends EventEmitter {
  ...
  build () {
    let theme = this.preflight()
    let outputDir = this.readYAML(path.join(process.cwd(), 'config.yml'))
      .publishDir || 'public'
  ...
  epub () {
    ...
    // console.log(this.chapterPaths())
    let epub = new Epub()
    epub.write()

    // Basic process:
    // 1. Run hugo server and webpack (see pdf() method)
    // 2. Get the chapters []
    // 3. Hit each chapter with some kind of parsing / scraping method:
    //    a. Grab an appropriate pre-gen epub template
    //    b. stuff the text content into the template
    //    c. write this file to a temporary output folder
    //    d. Make sure to copy image assets, modify links, etc.; use regex?
    //    e. Each asset should also be added to a shared manifest array
    // 4. Write the other files needed by the epub (toc.ncx, ops, etc)
    // 5. Zip the whole thing into an epub file and copy it to
    //      static/downloads/output.epub or whatever
  }
   ...
   */
  basePath () {
    return (this.readYAML('config.yml').baseurl || '').split('/').pop()
  }
  ...
  preflight () {
    if (this.isValidProject()) {
      let config = this.readYAML(path.join(this.currentDir, 'config.yml'))
      return {
        name: config.theme,
