...
class Epub extends EventEmitter {
  ...
  getPageContent (page) {
    ...
    $pageImgs.attr('src', (index, src) => {
      let localPath = path.resolve(path.join(this.outputDir, this.imageDir), src)
    ...
    $pageLinks
      .not(function (i, el) { return $(this).attr('href').match(/^#/) })
      .attr('href', (i, el) => { return this.convertToEpubLink(el) })

    // figure out the name of this page and supply it as href if possible
  ...
  convertToEpubLink (href) {
    if (href.match(/^http|https|www/)) { return href }
