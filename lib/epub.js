...
class Epub extends EventEmitter {
  ...
  buildEpubFiles(arr, title, EPUBdata) {
    ...
    let stream = fs.createWriteStream('html/epub.xhtml')
    ...
    if (!fs.existsSync('config/metadata.yml')) {
      let creators = ``
      arr.EPUBdata.creators.forEach((v, i) => {
        creators += `
- role: ${v.role}
text: ${v.name}`
      })

      let metaStream = fs.createWriteStream('html/metadata.yml')
      metaStream.once('open', function (fd) {
...
creator:
${creators}
publisher: ${arr.EPUBdata.publisher}
rights: ${arr.EPUBdata.rights}
language: ${arr.EPUBdata.languages}
date: ${arr.EPUBdata.date}
...
    `
        metaStream.end(metadataTemplate)
      })
    } else {
      fs.rename('config/metadata.yml', 'html/metadata.yml', () => { })
    }

    let templateStream = fs.createWriteStream('html/template.xhtml')
    templateStream.once('open', function (fd) {
      templateStream.end(pandocTemplate.template)
    })
