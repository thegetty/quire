...
class Build extends EventEmitter {
  ...
  createReformatedHashLinkArrAndcreateLinkArr(arr) {
    return new Promise(function (resolve, reject) {
      if (arr === '' || arr.length === 0) {
        reject(new Error('Array is empty'), null)
        removeHTML()
      } else {
  ...
  buildMetadata(arr) {
    return new Promise(function (resolve, reject) {
      if (arr === '' || arr.length === 0) {
        ...
        reject(new Error('Title is empty'), null)
        removeHTML()
      } else if (arr['language'] !== undefined && arr['language'] !== null && arr['language'] === '') {
        reject(new Error('Language is empty'), null)
        removeHTML()
      } else {
        delete arr.pages
        let meta = ``
  ...
  buildHTML(arr, linksTitles, hashLinks) {
    return new Promise(function (resolve, reject) {
      if (arr.pages === undefined || linksTitles === '' || hashLinks === '') {
        reject(new Error('Array is empty'), null)
        removeHTML()
      } else {
        let html = []
