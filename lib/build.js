    ...
    buildHTML(arr, linksTitles, hashLinks) {
        return new Promise(function (resolve, reject) {
            if (arr === '' || linksTitles === '' || hashLinks === '') {
                ...
                <html>
                  <head>
                    <title>${arr.title.replace(/[^a-zA-Z0-9 ]/g, '')}</title>
                  </head>
                  <body>
                    <div>
                      ${newHtml}
                ...
                updateHTML.find('figure')
                    .each((index, el) => {
                        ...
                        let figCaption = $(el).find('figcaption').html() !== undefined ? `${$(el).find('figcaption').html()}` : ''
                        return $(el).replaceWith(`<p><span ${imgId}></span></p><figure><img ${imgSrc}/> <figcaption>${figCaption}</figcaption></figure>`)
                    })
