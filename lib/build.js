...
// const pandocTemplate = require("epubTemplate");
const pandocTemplate = `<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE html>
  ...
  buildEpubFiles(arr) {
    return new Promise((resolve, reject) => {
      ...
      this.createReformatedHashLinkArrAndcreateLinkArr(arr)
        .then(r => {
          this.buildHTML(arr, r[0], r[1]).catch(err => {
            errorExit(err, this.spinner);
          });
        })
        .then(() => {
          this.buildMetadata(arr).catch(err => {
            errorExit(err, this.spinner);
          });
        })
        .then(() => {
