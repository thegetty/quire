...
class Project extends EventEmitter {
  constructor (verbose) {
    ...
    if (!this.config.publishDir) { this.config.publishDir = 'public' }
  ...
  preview () {
    ...
    this.hugo = this.spawnHugo('server', '--config=config.yml,config.dev.yml');
  ...
  buildWeb () {
    ...
    this.buildTheme()
      .then(() => {
        execFileSync(hugo, ['--config=config.yml,config.build.yml'], { stdio: 'inherit' })
  ...
  buildPDF () {
    ...
    let hugoServer = this.spawnHugo('server')
    let princeArgs = _
   ...
   */
  buildEpub () {
    let hugoServer = this.spawnHugo('server', '--renderToDisk', '--config=config.yml,config.epub.yml')
  ...
  spawnHugo () {
    ...
    // Output Hugo Info
    if (this.verbose) {
      hugoServer.stdout.on('data', function(buf) {
