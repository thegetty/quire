...
class Project extends EventEmitter {
  ...
  getURLforChapter (chapter) {
    ...
    // Figure out what the user has specified for their baseurl.
    // The result should be a string (including an empty string).
    try {
      baseURL = new URL(this.config.baseURL).pathname
    } catch (e) {
      if (e instanceof TypeError) {
        baseURL = path.parse(this.config.baseURL || '').name
      }
    }
