...
class Project extends EventEmitter {
  ...
  async buildPDF(file, env) {
    ...
    let princeArgs;
    if (!this.isWin()) {
      checkForCommand(princeCmd, spinner);
    }
  ...
  async buildEpub(file, env) {
    ...
    checkForCommand("pandoc", spinner);
    await checkForFileRemove(
    ...
    try {
      ...
      if (ready) {
        ...
        if (dirCreated) {
          ...
          await checkForFileRemove(`site/source.js`);
          await checkForFileRemove(
    ...
    try {
      ...
      if (ready) {
        ...
        if (dirCreated) {
          ...
          await checkForFileRemove(`${fileNamePath}-mobi.epub`);
          await checkForFileRemove(`site/source.js`);
          await checkForFileRemove(
            path.join("themes", this.theme, "static", "source.js")
          );
