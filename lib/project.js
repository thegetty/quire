    ...
    this.buildTheme()
      ...
      .then(() => {
        ...
        this.walk(dir, function (err) {
          ...
          console.log(`filepath: ${file}.epub`)
        })
      })
      .then(() => this.hugo.kill())
      .then(() => {
    ...
    this.buildTheme()
      ...
      .then(() => {
        ...
        this.walk(dir, function (err) {
          ...
          fileName = fileName !== undefined ? fileName : 'output'
          let file = `${filePath}${fileName}`
          let args = `-f html-native_divs-native_spans-raw_html-auto_identifiers -t epub html/epub.xhtml -o ${file}.epub --epub-metadata=html/metadata.yml --epub-cover-image=static/img/promo_image.png --template=html/template.xhtml  --standalone --css=site/css/epub.css`
          execSync(`pandoc ${args}`)
