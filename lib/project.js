...
class Project extends EventEmitter {
  ...
  buildPDF(env) {
    ...
    princeArgs.push('-o', 'static/downloads/output.pdf')
    this.buildTheme()
      .then(() => {
 ...
 * @description TODO
 */
  buildPDCEpub(fileName, filePath, env) {
    ...
    this.buildTheme()
      ...
      .then(() => {
        ...
        this.walk(dir, function (err) {
          ...
          let args = `-f html-native_divs-native_spans-raw_html -t epub-auto_identifiers html/epub.xhtml -o static/downloads/${fileName}.epub --epub-metadata=html/metadata.yml --epub-cover-image=static/img/promo_image.png --template=html/template.xhtml  --standalone --css=site/css/epub.css`
          execSync(`pandoc ${args}`)
          console.log(`filepath: static/downloads/${fileName}.epub`)
        })
      })
      .then(() => this.hugo.kill())
      .then(() => {
      ...
      .then(() => {
        ...
        this.walk(dir, function (err) {
          ...
          kindlegen(fs.readFileSync(`static/downloads/${fileName}.epub`), (error, mobi) => {
            ...
            writeFile(`static/downloads/${fileName}.mobi`, mobi)
            console.log(`filepath: static/downloads/${fileName}.mobi`)
            fs.unlinkSync(`static/downloads/${fileName}.epub`)
          })
        })
      })
      .then(() => this.hugo.kill())
