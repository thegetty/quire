    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then((output) => {
        ...
        this.checkFolderCreated(dir, err => {
          ...
          let file = `${filePath}${fileName}`
          let args = `-f html-native_divs+native_spans -t epub html/epub.xhtml -o ${file}.epub --epub-metadata=html/dc.xml ${cover} --template=html/template.xhtml --css=site/css/epub.css -s`
      ...
      .then((output) => {
        ...
        this.checkFolderCreated(dir, err => {
          ...
          kindlegen(fs.readFileSync(`static/downloads/${fileName}.epub`), (error, mobi) => {
            if (error) throw spinner.fail([`${error}`])
