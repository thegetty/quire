...
class Project extends EventEmitter {
  ...
  buildPDF(file, env) {
    ...
    PDF(bookData, spinner)
      .then(() => {
        hugoServer = this.spawnHugo('server', '--config=' + configs.join(','))
    ...
    this.buildTheme(`webpack.config.pdf.js`)
      .then(() => {
        ...
        // spinner.info([`Command: ${princeCmd} ${princeArgs.toString().replace(/,/g, ' ')} -o ${fileNamePath}.pdf --no-warn-css-unsupported --no-warn-css-unknown --no-warn-css`])
        hugoServer.kill()
    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then((output, server) => {
        ...
        bookData.chapters = this.chapters
          .filter((c) => {
            let chapter = frontmatter.loadFront(c);
            if (chapter.epub === false) {
              chapter[]
            }
            console.log(chapter)
            return chapter.epub !== false
          })
