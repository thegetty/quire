...
const { commandMissing, readYAML, determineBaseURL } = require("./utils");
...
class Project extends EventEmitter {
  ...
  buildEpub(file, env) {
    return new Promise(resolve => {
      ...
      this.buildTheme(`webpack.config.epub.js`, baseURL)
        .then(() => {
          return this.hugo.ready;
        })
        .then(() => {
        ...
        .then(output => {
          ...
          this.checkFolderCreated(dir, err => {
            ...
            let cover =
              output.data.cover !== undefined
                ? output.data.cover.replace("http://localhost:1313/img/", "")
                : "";
            cover =
              cover !== "" ? `--epub-cover-image=static/img/${cover}` : "";
            let fileNamePath = `${filePath}/${fileName}`;
    ...
    return new Promise(resolve => {
      ...
      this.buildTheme(`webpack.config.epub.js`, baseURL)
        ...
        .then((output, server) => {
          let bookData = this.loadBookData();
          let configData = this.config;
          bookData.chapters = _.filter(this.chapters, c => {
        ...
        .then(output => {
          ...
          this.checkFolderCreated(dir, err => {
            ...
            execSync(`pandoc ${args}`);
            kindlegen(
              fs.readFileSync(`${fileNamePath}-mobi.epub`),
              (error, mobi) => {
      ...
      list.forEach(file => {
        ...
        fs.stat(file, (err, stat) => {
          if (stat && stat.isDirectory()) {
            this.checkFolderCreated(file, (err, res) => {
