...
class Project extends EventEmitter {
  ...
  preview () {
    ...
    execFile(hugo, ['server', '--config=config.yml,config.dev.yml'])
  ...
  buildPDF () {
    ...
    let hugoServer = execFile(hugo, ['server'])
    let princeArgs = _
   ...
   */
  buildEpub () {
    let hugoServer = execFile(hugo, ['server', '--renderToDisk', '--config=config.yml,config.epub.yml'])
