...
class Project extends EventEmitter {
  ...
  preview(webpackConfig) {
    ...
    this.hugo = this.spawnHugo('server', '--config=config.yml,config/environments/dev.yml', '--watch')
  ...
  buildWeb(env) {
    ...
    let getURL = this.getBaseUrl(configs) !== "" ? this.getBaseUrl(configs) : '/'
    let baseURL = getURL[0] !== undefined ? getURL[0].baseURL : '/'

    let spinner = ora({
      text: 'Building theme assets...'
    }).start();

    this.buildTheme(`webpack.config.prod.js`, baseURL)
      .then(() => {
  ...
  buildEpub(file, env) {
    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then(() => {
        fs.copy(path.join('themes', this.theme, 'static', 'css', 'epub.css'), path.join(this.config.publishDir, 'css', 'epub.css'), err => {
