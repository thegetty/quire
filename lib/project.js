...
class Project extends EventEmitter {
  ...
  preview () {
    ...
    spawn('hugo', ['server', '--config=config.yml,config.dev.yml'])
  ...
  buildWeb () {
    ...
    this.buildTheme()
      .then(() => {
        spawnSync('hugo', ['--config=config.yml,config.build.yml'], { stdio: 'inherit' })
  ...
  buildEpub () {
    this.checkForCommand('hugo')
    let hugo = spawn('hugo', ['server', '--renderToDisk', '--config=config.yml,config.epub.yml'])
