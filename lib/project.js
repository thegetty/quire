 ...
 * @author Nick Hulea / Getty Publications
 * @license MIT
 *
 */
...
class Project extends EventEmitter {
   ...
   */
  buildPDF(file, env) {
    this.checkForFileRemove(path.join('themes', this.theme, 'static', 'css', 'application.css'))
    ...
    let defaultName = bookData.short_title !== null ? bookData.short_title : 'output'

    file = file !== undefined ? file : `static/downloads/${defaultName}`
    ...
    fs.ensureDir(filePath).catch(e => {
      this.errorExit(`${e}`, spinner)
    })

    filePath = filePath !== undefined && filePath !== '' ? filePath : 'static/downloads'
    fileName = fileName !== undefined && fileName !== '' ? fileName : defaultName

    let fileNamePath = `${filePath}/${fileName}`

    PDF(bookData, spinner)
      .catch(e => {
        this.errorExit(`${e.message}`, spinner)
      })

    this.buildTheme(`webpack.config.pdf.js`)
      .then(() => {
        this.hugo = this.spawnHugo('server', '--config=' + configs.join(','))
        return this.hugo
      })
      .then(() => {
      ...
      .then(() => {
        fs.ensureFile(path.join('themes', this.theme, 'static', 'css', 'application.css'))
          .then(() => {
            ...
            spinner.info([`Execution Time: ${((performance.now() - start) / 1000).toFixed(3) } seconds`])

            process.on('exit', function () {
              // console.log(prince.BYTES_PER_ELEMENT);
            });
    ...
    /*     PDF(bookData, spinner)
        ...
        this.buildTheme(`webpack.config.pdf.js`)
          .then(() => {
            fs.ensureFile(path.join('themes', this.theme, 'static', 'css', 'application.css'))
              .then(() => {
                runPrince(princeCmd, princeArgs, fileNamePath)
                  .then(() => {})
                  .catch(e => {
                    this.errorExit(`${e.message}`, spinner)
                  })
    ...
    PDF(bookData, spinner)
        ...
        this.buildTheme(`webpack.config.pdf.js`)
          .then(() => {
            let filePath = path.join('themes', this.theme, 'static', 'css', 'application.css')
  ...
  buildMobi(file, env) {
    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then((output, server) => {
        ...
        let configData = this.config
        bookData.chapters = _
          .filter(this.chapters, c => {
            if (frontmatter.loadFront(c).epub === false) {
              frontmatter.loadFront(c).toc = false
            }
            console.log(frontmatter.loadFront(c))
            return frontmatter.loadFront(c).epub !== false
          })
          .map(c => this.getURLforChapter(c))
        let ebook = new Epub(bookData, configData)
