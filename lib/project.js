...
class Project extends EventEmitter {
  ...
  buildPDF(fileName, filePath, env) {
    ...
    this.buildTheme(`webpack.config.dev.js`)
      .then(() => {
 ...
 * results in an EPUB. The cover image is taken from
 * 
 */
    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then((output) => {
        ...
        this.checkFolderCreated(dir, err => {
          ...
          let file = `${filePath}${fileName}`
          let args = `-f html-native_divs-native_spans-raw_html+auto_identifiers+footnotes -t epub+footnotes html/epub.xhtml -o ${file}.epub --epub-metadata=html/metadata.yml ${cover} --template=html/template.xhtml  --standalone --css=site/css/epub.css --ascii`
    ...
    this.buildTheme(`webpack.config.epub.js`)
      ...
      .then((output) => {
        ...
        this.checkFolderCreated(dir, err => {
          ...
          cover = cover !== '' ? `--epub-cover-image=static/img/${cover}` : ''
          let file = `${filePath}${fileName}`
          let args = `-f html-native_divs-native_spans-raw_html+auto_identifiers -t epub html/epub.xhtml -o ${file}.epub --epub-metadata=html/metadata.yml ${cover} --template=html/template.xhtml  --standalone --css=site/css/epub.css --ascii`
          execSync(`pandoc ${args}`)
